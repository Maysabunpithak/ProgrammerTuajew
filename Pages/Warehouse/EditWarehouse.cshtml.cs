...
namespace ProgrammerTuajew.Pages.Warehouse
{
    ...
    {
        public WarehouseInfo SaleInfo = new WarehouseInfo();
        ...
        public void OnGet()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    string sql = "SELECT Id, Sender, Receiver, ReceiverDepartment, Subject, Body, DateSent FROM SaleD1 WHERE Id = @Id AND SenderDepartment = 'Warehouse'";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            {
                                ErrorMessage = "No record found with the given ID for Warehouse.";
                            }
                        }
                    }
                }
        ...
        public void OnPost()
        {
            ...
            string viewType = Request.Query["view"]; // รับค่า view จาก Query String (inbox หรือ sent)
            ...
            try
            {
                ...
                {
                    connection.Open();
                    string sql = "UPDATE SaleD1 SET Subject = @Subject, Body = @Body, DateSent = @DateSent WHERE Id = @Id AND SenderDepartment = 'Warehouse'";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            Response.Redirect($"/Warehouse/IndexWarehouse?view={viewType}");
