...
{
    public class EditSaleD2Model : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    string sql = "SELECT Id, Sender, Receiver, ReceiverDepartment, Subject, Body, DateSent FROM SaleD1 WHERE Id = @Id AND SenderDepartment = 'SaleD2'";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            {
                                ErrorMessage = "No record found with the given ID for SaleD2.";
                            }
                        }
                    }
                }
        ...
        public void OnPost()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    string sql = "UPDATE SaleD1 SET Subject = @Subject, Body = @Body, DateSent = @DateSent WHERE Id = @Id AND SenderDepartment = 'SaleD2'";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            Response.Redirect($"/SaleD2/IndexSaleD2?view={viewType}"); // เด้งกลับไปยังหน้าเดิม (view=sent หรือ view=inbox)
                        }
                        else
                        {
